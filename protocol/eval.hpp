...
class DtEval{
    ...
    DtEval(Dt<T, Node<T>> * dt, Shuffler<T> shuffler, RepShare<std::vector<T>> features):dt(dt), shuffler(shuffler), features(features){
    ...
    void offline(){
        ...
        RepShare<std::vector<uint32_t>> delta_n{
            std::vector<uint32_t>(2 * dt->node_lens + 1, 0),std::vector<uint32_t>(2 * dt->node_lens + 1, 0)};
        RepShare<std::vector<uint32_t>> delta_f{
            std::vector<uint32_t>(2 * dt->node_lens + 1, 0),std::vector<uint32_t>(2 * dt->node_lens + 1, 0)};

        delta_n.shares[0][0] = rep_perms_n.shares[0][0] % dt->node_lens; delta_n.shares[1][0] = rep_perms_n.shares[1][0] % dt->node_lens;
        for(int i = 1; i < dt->node_lens; i++){
            delta_n.shares[0][i] =(rep_perms_n.shares[0][i] - rep_perms_n.shares[0][i-1] + dt->node_lens) % dt->node_lens; delta_n.shares[1][i] = (rep_perms_n.shares[1][i] - rep_perms_n.shares[1][i-1] + dt->node_lens) % dt->node_lens;
        }

        delta_f.shares[0][0] = rep_perms_f.shares[0][0] % dt->data_lens; delta_f.shares[1][0] = rep_perms_f.shares[1][0] % dt->data_lens;
        for(int i = 1; i < dt->data_lens; i++){
            delta_f.shares[0][i] = (rep_perms_f.shares[0][i] - rep_perms_f.shares[0][i-1]+ dt->data_lens) % dt->data_lens; delta_f.shares[1][i] = (rep_perms_f.shares[1][i] - rep_perms_f.shares[1][i-1]+ dt->data_lens) % dt->data_lens;
        }
        ...
        for(int i = 1; i < 2 * dt->node_lens + 1; i++){
            delta_n.shares[0][i] = (delta_n.shares[0][i] + delta_n.shares[0][i-1]) % dt->node_lens;
            delta_n.shares[1][i] = (delta_n.shares[1][i] + delta_n.shares[1][i-1]) % dt->node_lens;
        }
        ...
        //对delta_f求 前缀和
        for(int i = 1; i < 2 * dt->node_lens + 1; i++){
            delta_f.shares[0][i] = (delta_f.shares[0][i] + delta_f.shares[0][i-1]) % dt->data_lens;
            delta_f.shares[1][i] = (delta_f.shares[1][i] + delta_f.shares[1][i-1]) % dt->data_lens;
        }
        ...
        for(int i = 0; i < dt->node_lens; i++){
            dt->Ps.shares[0][i].ln = delta_n.shares[0][2*i+1]; dt->Ps.shares[1][i].ln = delta_n.shares[1][2*i+2];
            dt->Ps.shares[0][i].rn = delta_n.shares[0][2*i+1]; dt->Ps.shares[1][i].rn = delta_n.shares[1][2*i+2];
            dt->Ps.shares[0][i].lf = delta_f.shares[0][2*i+1]; dt->Ps.shares[1][i].lf = delta_f.shares[1][2*i+2];
            dt->Ps.shares[0][i].rf = delta_f.shares[0][2*i+1]; dt->Ps.shares[1][i].rf = delta_f.shares[1][2*i+2];
        }
        shuffler.shuffle(dt->Ps, perms_n, dt->node_lens);
        
        RepShare<uint32_t> tmp_n, tmp_f;
        tmp_n.shares[0] = delta_n.shares[0][0]; tmp_n.shares[1] = delta_n.shares[1][0];
        tmp_f.shares[0] = delta_f.shares[0][0]; tmp_f.shares[1] = delta_f.shares[1][0];
    ...
    T online(){
        ...
        for(int i = 0; i < dt->depth; i++){
            ...
            ln_share.shares[0] = cur_node.shares[0].ln; ln_share.shares[1] = cur_node.shares[1].ln;
            rn_share.shares[0] = cur_node.shares[0].rn; rn_share.shares[1] = cur_node.shares[1].rn;
            lf_share.shares[0] = cur_node.shares[0].lf; lf_share.shares[1] = cur_node.shares[1].lf;
            rf_share.shares[0] = cur_node.shares[0].rf; rf_share.shares[1] = cur_node.shares[1].rf;
